...
void CGameFramework::ProcessInput()
{
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		플레이어의 이동 속력은 (10000/초)로 가정한다.*/
		if (dwDirection)
		{
			m_pPlayer->Move(dwDirection, 10000.0f * m_GameTimer.getTimeElapsed(),true);
			auto terrainPos = m_pScene->GetShaders()[1]->GetObjects()[0]->GetPosition();
			//비행기와 지형의 거리가 150.0만큼 차이나면 지형을 옮긴다.
			if (fabs(terrainPos.z+450.0f - m_pPlayer->GetPosition().z) > 150.0f)
			{
				if (terrainPos.z + 450.0f > m_pPlayer->GetPosition().z)
				{
					m_pScene->GetShaders()[1]->GetObjects()[0]->AddPosition(0.0f, 0.0f, -150.0f);
					m_pScene->GetShaders()[1]->GetObjects()[1]->AddPosition(0.0f, 0.0f, -150.0f);
					m_pScene->GetShaders()[1]->GetObjects()[2]->AddPosition(0.0f, 0.0f, -150.0f);
					m_pScene->GetShaders()[1]->GetObjects()[3]->AddPosition(0.0f, 0.0f, -150.0f);
				}
				else
				{
					m_pScene->GetShaders()[1]->GetObjects()[0]->AddPosition(0.0f, 0.0f, 150.0f);
...
void CGameFramework::WaitForGpuComplete()
{
	...
	{
		hResult = m_pd3dFence->SetEventOnCompletion(nFence, m_hFenceEvent); //펜스값이 도달하면 실행시킬 이벤트 지정
		::WaitForSingleObject(m_hFenceEvent, INFINITE);//지정된 객체가 신호 상태에 있거나 시간 초과 간격이 경과 할 때까지 기다립니다.
		//여기선 인피니트로해서 계속 기다리게했음. 신호끝나기전까지.
	}
}
