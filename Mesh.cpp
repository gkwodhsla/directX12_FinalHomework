		...
		pd3dCommandList)
{
	...
	m_boundingSphere.Radius = fWidth / 2.0f;
	//바운딩구를 나타낸다.
...
CPlaneMeshDiffused::CPlaneMeshDiffused(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	int perWidth = 20;
	int perHeight = 20;
	int widthPointNum = width / perWidth;
	int heightPointNum = height / perHeight;
	m_nVertices = widthPointNum * heightPointNum;
	m_nStride = sizeof(CDiffusedVertex);
	m_nOffset = 0;
	m_nSlot = 0;
	...
	int k = 0;
	for (int i = 0; i < heightPointNum; ++i)
	{
		for (int j = 0; j < widthPointNum; ++j)
		{
			pVertices[k++] = CDiffusedVertex(XMFLOAT3(perWidth*j,0.0f,perHeight*i), RANDOM_COLOR);
		}
	}
	m_pd3dVertexBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, pVertices, m_nStride * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dVertexUploadBuffer);
	m_d3dVertexBufferView.BufferLocation = m_pd3dVertexBuffer->GetGPUVirtualAddress();
	...
	m_nIndices = ((widthPointNum * 2) * (heightPointNum - 1)) + ((heightPointNum - 1) - 1);
	UINT *pIndices = new UINT[m_nIndices];

	for (int j = 0, z = 0; z < heightPointNum - 1; ++z)
	{
		if (z % 2 == 0)
		{
			for (int x = 0; x < widthPointNum; ++x)
			{
				if ((x == 0) && (z > 0)) pIndices[j++] = UINT(x + (z * widthPointNum));
				pIndices[j++] = UINT(x + (z * widthPointNum));
				pIndices[j++] = UINT((x + (z * widthPointNum) + widthPointNum));
			}
		}
		else
		{
			for (int x = widthPointNum - 1; x >= 0; --x)
			{
				if (x == (widthPointNum-1))pIndices[j++] = UINT(x + (z * widthPointNum));
				pIndices[j++] = (UINT)(x + (z * widthPointNum));
				pIndices[j++] = (UINT)(x + (z * widthPointNum) + widthPointNum);
			}
		}
